...
class Trainer(StateDictMixin):
    def __init__(self, cfg: DictConfig, root_dir: Path) -> None:
        ...
        try:
            type(self.agent.denoiser).device = property(lambda self: next(self.parameters()).device)
            print(">>> Denoiser .device property patched successfully.")
        except Exception:
            # If the property already exists or can't be set, we manually inject it into the dict
            self.agent.denoiser.__dict__['device'] = self._device
        # --- DiT REPLACEMENT START ---
        try:
            from models.diffusion.inner_model import DiT
            
            print(">>> Replacing U-Net with DiT...")
            
            # Create DiT with same parameters
            dit = DiT(
                in_channels=cfg.agent.denoiser.inner_model.img_channels,
            ...
            self.agent.denoiser.inner_model = dit
            
            print(f">>> DiT REPLACEMENT SUCCESSFUL: {count_parameters(dit)} parameters")
            
        except Exception as e:
            import traceback
            print(f">>> DiT REPLACEMENT FAILED: {e}")
            traceback.print_exc()
            print(">>> Continuing with original U-Net")
        # --- DiT REPLACEMENT END ---
        ...
        def build_opt(name: str) -> torch.optim.AdamW:
            ...
            opt_cfg = getattr(cfg, name).optimizer
            
            # For denoiser with DiT, use simple optimizer
            if name == "denoiser":
                ...
                return configure_opt(model, **opt_cfg)
